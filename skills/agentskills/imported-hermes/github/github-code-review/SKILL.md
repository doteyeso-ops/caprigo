...
- **Submitting reviews:** Use **`gh`** from **`execute_command`** when installed; otherwise **`http_request`** `POST` to **`/repos/{owner}/{repo}/pulls/{pull_number}/reviews`** with JSON **`event`** (`APPROVE`, `REQUEST_CHANGES`, `COMMENT`) and **`body`** per [GitHub API](https://docs.github.com/rest/pulls/reviews).
- **Inline comments:** Create review comments via **`POST .../pulls/comments`** with **`commit_id`**, **`path`**, **`position`** or **`line`** — easier with **`gh api`** than raw JSON; **`execute_command`** is fine for `gh`.
- **Permissions:** Token needs **`pull_requests: write`** (classic PAT) or equivalent fine-grained scopes.
