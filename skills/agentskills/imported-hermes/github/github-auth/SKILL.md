...
- **Token storage:** Caprigo Core reads **`GITHUB_TOKEN` / `GH_TOKEN` from the gateway process environment** (how you launch the gateway, OS user env, or a `.env` next to the Caprigo install — not necessarily `~/.hermes/.env`). You can also keep secrets in **`CAPRIGO_*`** env vars if you wire them in your launcher.
- **Data root:** User data defaults to **`~/.caprigo`** (override with **`CAPRIGO_HOME`**). Hermes paths like `~/.hermes/.env` are only relevant if you still run Hermes alongside Caprigo.
- **`execute_command`** runs in the same environment as the gateway host; **`http_request`** is ideal for GitHub REST when you want to avoid shell quoting — pass `Authorization: Bearer <token>` or `token <PAT>` per GitHub docs.
