...
- **Prefer `gh`** for **secrets, variables, and Actions** — GitHub requires **Libsodium sealed box** encryption for repo secrets via the API; `gh secret set` avoids hand-rolling crypto. Use **`execute_command`** with `gh` when available.
- **Repo CRUD** works well via **`http_request`** (`POST/PATCH /user/repos`, `/repos/{owner}/{repo}`) with a PAT in the gateway environment.
- **Fork + clone:** Operations apply to the **workspace** the Caprigo gateway can access; cloning a huge repo may need a larger timeout or user-run steps outside the gateway.
