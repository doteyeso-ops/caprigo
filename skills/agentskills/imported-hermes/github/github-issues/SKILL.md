...
- **From chat (no local `gh`):** Use **`http_request`** with `GET`/`POST`/`PATCH` to `https://api.github.com/...` and header **`Authorization: Bearer $GITHUB_TOKEN`** (or `token` scheme per GitHub). Set **`Accept: application/vnd.github+json`**.
- **From shell:** Same curl snippets as above via **`execute_command`** — ensure the gateway process has **`GITHUB_TOKEN`** set if you rely on `$GITHUB_TOKEN` in scripts.
- **Listing/search:** Prefer **`http_request`** for small reads to keep output in the transcript; use **`execute_command`** when you need **`gh`**’s interactive UX or **`jq`** pipelines.
