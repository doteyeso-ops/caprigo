...
- Same mapping as **`github-issues`**: **`http_request`** for GitHub REST **`/pulls`**, **`/issues`**, reviews, and comments when **`gh`** is unavailable; use **`Authorization: Bearer`** and **`Accept: application/vnd.github+json`**.
- **Large diffs / patches:** `GET` with `Accept: application/vnd.github.diff` via **`http_request`** or local **`git`** via **`execute_command`** after checkout.
- **Fork workflows:** All `git` operations run in the workspace the gateway sees — confirm **`cwd`** and remotes match the user’s intent.
