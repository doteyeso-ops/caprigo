...
- **pygount / cloc / tokei:** Run via **`execute_command`** from the **workspace root** (set **`cwd`** if the repo is nested). Ensure the tool is installed on the **gateway machine**.
- **Hermes path assumptions:** Replace any `~/.hermes/...` artifact paths with paths **inside the project** or Caprigo’s data root (`CAPRIGO_HOME`, default `~/.caprigo`) when relevant.
- **Summaries:** Use **`list_directory`** + **`search_files`** first to avoid scanning `node_modules`, `.git`, `venv` — align with this doc’s `--folders-to-skip` advice.
