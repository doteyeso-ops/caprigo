...
**Caprigo Core today does not ship the Hermes `hermes webhook` CLI or dynamic subscription store.** Treat the sections above as the **Hermes reference design**. In Caprigo you can approximate the same outcomes by:

1. **External bridge:** n8n, Zapier, Cloudflare Workers, or a tiny service that receives provider webhooks and calls Caprigo’s **HTTP API** (or runs a script) to start a session / post a message — use your deployment’s auth story.
2. **MCP:** Some providers expose MCP servers; connect them under **Settings → MCP servers** and handle events with tools (`mcp_*`) instead of Hermes routes.
3. **Still need Hermes webhooks?** Run a **Hermes gateway** for webhook ingress and keep Caprigo for day-to-day chat, or track a future Caprigo feature for first-class subscriptions.

The generic security and payload-template ideas above remain useful when you implement a bridge.
...
**Note:** The Hermes-specific `hermes webhook *` commands in this skill are **not** available in Caprigo Core — see **Caprigo playbook (extended)** above.
