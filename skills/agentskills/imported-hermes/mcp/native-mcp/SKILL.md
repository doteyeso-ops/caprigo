...
Caprigo Core ships a built-in MCP **client** in the gateway (stdio servers; tools exposed as `mcp_<serverId>_<toolName>`).

1. Use **Settings → MCP servers** (or `GET`/`PATCH /api/mcp-servers`) — not `~/.hermes/config.yaml`.
2. Each server entry uses `command` + `args` (one per line in the UI) plus optional `env` / `cwd`.
3. After **Save & connect**, tools appear in chat; optional Windows desktop automation via **Windows-MCP** (`uvx windows-mcp`) follows the same path.

Hermes-specific prefixes like `mcp_time_*` become `mcp_<yourServerId>_*` in Caprigo.
