...
- **You are reading Hermes *upstream* developer docs.** In **Caprigo Core**, the equivalent runtime lives in **`packages/*`** (gateway, agent, user-skills-loader). There is no `hermes` CLI in Caprigo unless you install Nous Hermes separately.
- **Contributing patterns:** New capabilities belong in **`@caprigo/agent`** (tools/skills), **`@caprigo/gateway`** (HTTP/MCP), and **`skills/agentskills/`** — follow this repo’s TypeScript layout, not Python `hermes_constants` paths.
- **Config:** Use **`CAPRIGO_*` env**, **`CAPRIGO_HOME`**, and gateway **`PATCH /api/config`** — not `~/.hermes/config.yaml`, unless you run Hermes itself.
