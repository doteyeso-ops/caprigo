...
- **Shell:** Use **`execute_command`** with the **repo as `cwd`**. Caprigo does **not** mirror Hermes’ `terminal(..., pty=true)` helper name-for-name — if Codex **requires a TTY**, run it in a context that provides one (user terminal, Windows Terminal, or a wrapper script); non-interactive **`codex exec`** is the most reliable from the gateway.
- **Background / poll:** If the skill references Hermes **`process`** / **`poll`** tools, approximate with **`execute_command`** (start job) + log files, or ask the user to run long Codex sessions locally.
- **Secrets:** Pass **`OPENAI_API_KEY`** / provider keys via the **gateway environment**, not `~/.hermes/.env`, unless you symlink or load the same file in your launcher.
