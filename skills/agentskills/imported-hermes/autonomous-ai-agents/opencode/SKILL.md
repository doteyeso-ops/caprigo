...
- **Prefer `opencode run`** (or equivalent one-shot) via **`execute_command`** so the gateway gets a clean exit code and stdout/stderr.
- **Interactive TUI:** Same caveat as Codex — **`execute_command`** may not attach a full PTY; for long interactive sessions, prefer the **user’s terminal** or document manual steps.
- **Workspace:** Set **`cwd`** to the git repo root; Caprigo’s sandbox is whatever directory the gateway was started from / configured workspace.
