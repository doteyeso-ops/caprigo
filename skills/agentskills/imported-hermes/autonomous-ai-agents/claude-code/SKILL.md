...
- **Orchestration:** Run **`claude`** via **`execute_command`** with explicit **`cwd`** (git repo). On **Windows**, translate bash/tmux examples to **PowerShell** or **WSL** as appropriate — tmux may not exist on bare Windows.
- **Print mode (`-p`)** and **`claude exec`**-style flows work best **non-interactively** from Caprigo; full TUI orchestration may need a **local terminal** session.
- **Monitoring:** Replace Hermes-specific **`process` / `poll`** with **log tailing** (`Get-Content -Tail`) or asking the user to paste output when the gateway cannot attach to the session.
