...
- **Red/green/refactor** maps directly to **`execute_command`** (test runner), **`read_file`** / **`write_file`** / **`search_files`** for code and tests.
- **CI parity:** If the doc mentions Hermes CI hooks, run the **same commands locally** via `execute_command` (`npm test`, `pytest`, `cargo test`, etc.) from the repo root; set `cwd` when the project requires a subdirectory.
- **Coverage / watch mode:** Prefer one-shot commands suitable for an automated turn; avoid interactive TUI test runners unless the user asks.
