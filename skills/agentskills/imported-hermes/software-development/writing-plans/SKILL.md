...
- **Header template:** Replace “**For Hermes:** use subagent-driven-development …” with **Caprigo:** same workflow ideas apply; implementation is either **one session** (you execute tasks in order) or **fleet mode** (orchestrator + chained task agents — see `subagent-driven-development` Caprigo section).
- **Save location:** Prefer **`.caprigo/plans/`** or **`.hermes/plans/`** under the active workspace (both are valid); Caprigo file tools resolve paths from the configured workspace root.
- **Tooling:** Plans reference **`read_file`**, **`write_file`**, **`execute_command`**, **`search_files`** — there is no separate Hermes “backend” path; the gateway host is your shell environment.
