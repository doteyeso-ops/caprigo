...
- **delegate_task returns non-JSON** — retry once with stricter prompt, then treat as FAIL (Hermes); in Caprigo use the playbook below instead
...
- **No `delegate_task`:** Perform the “reviewer” role **in-process**: gather diff via `execute_command` (`git diff`, `git status`) or `read_file` on changed paths, then apply this skill’s checklist in a dedicated assistant turn. For a **second opinion**, use a **separate chat session** or a **chained task agent** and **`fleet_message`** with the review prompt.
- **Diff size:** Same splitting rules apply; Caprigo transcripts benefit from **per-file** reviews when diffs are huge.
- **Automation:** `execute_command` runs on the gateway host — ensure `git`, linters, and test runners are installed there (not only on the user’s laptop if those differ).
