...
- **Observe:** Use **`read_file`**, **`list_directory`**, **`search_files`**, and **`execute_command`** to reproduce (run minimal repro commands, tail logs, print env vars). Hermes-specific log paths (e.g. under `~/.hermes/`) map to **your app’s** logs or **`CAPRIGO_HOME`** / gateway output if debugging Caprigo itself.
- **Hypothesis / test:** Prefer **non-interactive** commands so each turn returns structured output you can quote back.
- **Binary / services:** If the bug needs a long-running server, describe what the user should run locally; Caprigo’s `execute_command` is better for **one-shot** probes.
