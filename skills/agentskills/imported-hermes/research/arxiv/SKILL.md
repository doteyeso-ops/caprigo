...
- Replace **`web_extract(urls=[...])`** with **`http_request`** (GET the abstract/PDF URL) or **`execute_command`** using `curl` / `Invoke-WebRequest`. For HTML pages, the raw body is often enough for the model to read; for PDFs you may need local extraction (`pdftotext`, Python) via shell.
- **arXiv Atom API** and **Semantic Scholar** examples in this doc work unchanged via **`execute_command`** (curl) or **`http_request`** — prefer the same URLs and query strings.
- Optional **`scripts/search_arxiv.py`** referenced above lives in the upstream Hermes repo; either copy that script into your workspace or rely on the inline **python3 -c** XML snippets via `execute_command`.
