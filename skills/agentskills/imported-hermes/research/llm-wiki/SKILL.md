...
- **Wiki on disk:** Treat the wiki tree as **normal repo files** — **`read_file`**, **`write_file`**, **`list_directory`**, **`search_files`**. Paths like `~/.hermes/...` in older examples should become paths **inside the user project** (e.g. `docs/wiki/`) unless you intentionally store under **`CAPRIGO_HOME`**.
- **`store_memory`:** Caprigo Core also offers **`store_memory` / `retrieve_memory` / `list_memory_keys`** for cross-session facts; use alongside markdown wiki pages as needed.
- **Lint / log rotation:** Implement with **`execute_command`** (scripts) or manual edits — no Hermes-specific wiki daemon.
