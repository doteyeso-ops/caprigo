...
- **CLI:** Install **`blogwatcher-cli`** on the **gateway machine**, then invoke via **`execute_command`**. Default DB path `~/.blogwatcher-cli/...` refers to the **gateway user’s home**, not necessarily the user’s desktop — override with **`--db`** or **`BLOGWATCHER_DB`** pointing inside the workspace if you want artifacts co-located with the project.
- **Feeds / scrape:** Same commands as upstream; Caprigo adds no special RSS layer — use **`http_request`** only if you reimplement fetch without the CLI.
