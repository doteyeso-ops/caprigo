...
- **Literature / metadata:** Use **`http_request`** for Semantic Scholar, CrossRef, arXiv, and venue pages; use **`execute_command`** when the doc suggests **curl** or **Python** one-liners.
- **Drafts:** Write LaTeX/Markdown with **`write_file`** under the workspace; Hermes-specific “upload to Hermes workspace” steps map to **Caprigo’s workspace root** (whatever the gateway exposes).
- **Figures / builds:** Run **`pdflatex`**, **`latexmk`**, or **`tectonic`** via **`execute_command`** on the gateway host if the toolchain is installed.
