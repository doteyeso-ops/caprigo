...
- **HTTP:** Polymarket **Gamma/CLOB/Data** endpoints in this skill map to **`http_request`** (GET/POST as documented) with the same URLs, headers, and query params.
- **Read-only:** This matches Caprigo’s model — no wallet signing in core; for trading flows the user must use their own wallet tooling outside the gateway.
- **Rate limits:** Respect the published limits; add backoff or caching if you script repeated calls from **`execute_command`**.
